...
## [1.0.14] — 开源代码改为可选 (2026-07-20)

- `index.html` §03 最终交付物:「开源代码 + 文档」由必交改为<b>可选,不强制要求</b>;开源相关的
  private→public 提示改为「若开源」的条件说明。
- 同步 §03 完赛材料(30%)构成:由「开源代码包 + 演示视频」改为以 ≤3 分钟讲解 / 演示视频为准,
  开源代码可选,避免与最终交付物口径矛盾。

## [1.0.13] — 赛末截止后存量评测继续结算 (2026-07-20)

- `index.html` §05 公告置顶新增 7·20「提交截止 · 存量评测继续结算」:北京时间 7 月 20 日
  24:00 后关闭工件上传与新增评测提交;以截止前 `/api/submit` 成功返回「已排队」为有效提交口径。
- 明确截止前 queued / running 提交全部继续评测直至完成,后续产出的成绩仍更新个人最佳与排行榜,
  无需在截止前等待结果或重复提交。
- 首页及排行榜倒计时结束文案由含混的「已截止 / 榜单冻结」改为
  「提交已截止 · 排队评测继续,成绩持续更新」。
- 7·18 历史公告补充当前口径:评测安全墙钟已在 12h 基础上进一步上调至 24h。

## [1.0.12] — 排行榜公告新增 7·18「评测超时误判 · 致歉与修复」 (2026-07-18)

- `index.html` §05 排行榜「公告 NOTICE」卡:置顶新增 7·18 条(高亮),原 7·08 转灰为历史条目。
- 缘由:评测并发升高使单任务(尤其 T4 叠三碗)耗时拉长,触发此前 6 小时评测超时上限,部分评测被
  误判为「评测失败」。公告致歉并说明:超时上限已由 6h 上调至 12h(后端 `EVAL_MAX_TIMEOUT_S` /
  `worker.py max_timeout_s` 21600→43200),受影响提交由主办方统一重新入队重评,无需重复提交、不占当日次数。

## [1.0.11] — 排行榜公告新增「观测契约」提示 + 公告卡合并 (2026-07-08)

- `index.html` §05 排行榜:原两条日期公告合并为一张「公告 NOTICE」卡片(按日期分块、最新在上,
  7·08 高亮 / 7·07 保留)。
- 新增 7·08「线上评测观测契约」:线上评测观测仅 RGB 三相机(head / left / right)+ qpos,不含
  点云 / 深度;线上评测以代码包(starter kit)的 eval 脚本及其配置为准,提交前请用 kit 的 clean
  配置 + 公开种子(`starter/public_seeds.json`)本地自测通过。

## [1.0.10] — 正式提交每日限额 1 → 2 次 (2026-07-07)

- 每日正式提交上限「每日 1 次」→「每日 2 次」(后端 `DAILY_LIMIT` 1→2)。`index.html` / `doc.html`
  全部「每日 1 次」文案同步为「每日 2 次」(提交说明、机制表、429 错误码提示、赛期机时参考)。
- 与后端 `deploy/secrets.env`(`DAILY_LIMIT=2`)同步;此前 0704 曾 3→1。

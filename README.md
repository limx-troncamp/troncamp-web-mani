...
TRON 2 双臂机器人 + RoboTwin 仿真,单卡训 ACT(Action Chunking Transformer,~80M)完成四个难度递增的操作任务:
...
- `data/examples.json` — 6 条占位示例(持续同步时与后端真实榜合并;数字与公示阈值一致)。
  合并器与持续同步脚本随主办方后端仓走(运维细节不入本公开仓)。
...
Static single-page site (GitHub Pages) for the **TronCamp Mani hackathon**: TRON 2
dual-arm robot + RoboTwin simulation, single-GPU ACT policies, four tasks of
increasing difficulty (`adjust_bottle` / `grab_roller` / `stack_bowls_two` /
`stack_bowls_three`), sequentially unlocked; only T4 is scored (end-state graded
/100). `index.html` is the main page, `doc.html` the contestant guide;
`board.js` renders `data/leaderboard.json` published by the organizers' backend.
